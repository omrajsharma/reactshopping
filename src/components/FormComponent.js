...
export default function  FormComponent(){
    ...
    const[isUserVaild,setUserVaild] = useState(false);
    const[pwdMsg,setPwdMsg]=useState('');
    const[capsStatus,setCapsStatus] = useState(true);
...
function HandleUserChange(e){
    setUserDetails({UserId:e.target.value,
        Password:userDetails.Password,
        City:userDetails.City})
}

function HandlePasswordChange(e){
    setUserDetails({UserId:userDetails.UserId,
...
function RegisterClick(){
    ...
    return(
        <div className="container-fluid">
            ...
            <dl>
                <dt>USer Id</dt>
                <dd><input type="text" onChange={HideUserMsg} onKeyUp={VerifyUserId}/></dd>
                <dd className={(isUserVaild==true)?'text-success':'text-danger'}>{userMsg}</dd>
                <dt>Password</dt>
                <dd>
                    <input type="password" onChange={HandlePasswordChange} onBlur={{HidePasswordMsg}} onKeyUP={VerifyPassword} />
